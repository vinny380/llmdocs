...
def test_init_skips_existing_config_without_force(runner: CliRunner, tmp_path: Path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path) as fs:
...
def test_serve_calls_run_server(runner: CliRunner, config_dir: Path) -> None:
    with patch("llmdocs.server.run_server") as mock_run:
        result = runner.invoke(cli, [
            "serve",
            "--config", str(config_dir / "llmdocs.yaml"),
            "--data-dir", str(config_dir / "data"),
        ])
    assert result.exit_code == 0, result.output
...
def test_build_indexes_docs_and_writes_llms_txt(runner: CliRunner, config_dir: Path) -> None:
    result = runner.invoke(cli, [
        "build",
        "--config", str(config_dir / "llmdocs.yaml"),
        "--data-dir", str(config_dir / "data"),
...
def test_validate_missing_config_exits_nonzero(runner: CliRunner, tmp_path: Path) -> None:
...
def test_validate_passes_for_valid_docs(runner: CliRunner, config_dir: Path) -> None:
    result = runner.invoke(cli, ["validate", "--config", str(config_dir / "llmdocs.yaml")])
    assert result.exit_code == 0, result.output
    assert "no issues" in result.output.lower()


def test_validate_fails_when_description_missing(runner: CliRunner, tmp_path: Path) -> None:
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
